...
export default async function Page() {
  ...
  const brandsWithCount = BRAND_CONFIG.map((brand) => {
    const count = products.filter((p) => p.brand.toLowerCase() === brand.dataBrand).length;
    return { ...brand, count };
  });
  ...
  return (
    <>
      <header className="text-center mb-16">
        <h1 className="text-4xl font-extrabold text-neutral-900 mb-4">Marcas</h1>
        ...
        {brandsWithCount.map((brand) => (
          <Link key={brand.slug} href={`/marcas/${brand.slug}`}>
            <article className="group bg-white border border-neutral-200 p-6 rounded-2xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 h-full">
              <span className="text-xs font-bold uppercase tracking-widest text-yellow-600 mb-3 block">
                Marca
              </span>
              <h2 className="text-2xl font-bold text-neutral-800 group-hover:text-yellow-600 mb-3">
                {brand.name}
              </h2>
              <p className="text-sm text-neutral-500 min-h-[66px]">{brand.description}</p>
              <div className="flex justify-between items-center mt-6">
                <span className="text-neutral-400 font-medium">{brand.count} Productos</span>
